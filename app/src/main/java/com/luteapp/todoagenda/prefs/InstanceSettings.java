...
import com.luteapp.todoagenda.widget.Alignment;
...
import static com.luteapp.todoagenda.prefs.colors.ColorThemeType.*;
...
public class InstanceSettings {
    ...
    private InstanceSettings setFromJson(JSONObject json) {
        ...
        try {
            ...
            boolean differentColorsForDark = canHaveDifferentColorsForDark() && json.has(PREF_DARK_THEME);
            defaultColors = ThemeColors.fromJson(context,
                    differentColorsForDark ? LIGHT : SINGLE, json);
            darkColors = differentColorsForDark
                    ? ThemeColors.fromJson(context, DARK, json.getJSONObject(PREF_DARK_THEME))
                    : ThemeColors.EMPTY;
    ...
    private InstanceSettings setFromApplicationPreferences(InstanceSettings settingsStored) {
        ...
        switch (ApplicationPreferences.getEditingColorThemeType(context)) {
            case DARK:
                darkColors = new ThemeColors(context, DARK).setFromApplicationPreferences();
                defaultColors = settingsStored == null
                        ? darkColors
                        : settingsStored.defaultColors.copy(context, LIGHT);
                break;
            case LIGHT:
                defaultColors = new ThemeColors(context, LIGHT).setFromApplicationPreferences();
                darkColors = settingsStored == null
                    ? defaultColors
                    : settingsStored.darkColors.isEmpty()
                        ? settingsStored.defaultColors.copy(context, DARK)
                        : settingsStored.darkColors.copy(context, DARK);
                break;
            case SINGLE:
                darkColors = ThemeColors.EMPTY;
                defaultColors = new ThemeColors(context, SINGLE).setFromApplicationPreferences();
                break;
            case NONE:
                darkColors = ThemeColors.EMPTY;
                defaultColors = settingsStored == null
                        ? darkColors
                        : settingsStored.defaultColors.copy(context, SINGLE);
                break;
        }
    ...
    public InstanceSettings(Context context, int widgetId, String proposedInstanceName) {
        ...
        defaultColors = context == null ? ThemeColors.EMPTY : new ThemeColors(context, SINGLE);
    }
